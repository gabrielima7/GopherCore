...
// Constraints: None.
// Errors: None.
// Thread-safety: All methods on Result are strictly safe for concurrent use since the type is entirely immutable by design after creation.
...
//
// Purpose: Successful initialization.
...
//
// Purpose: Failure initialization.
...
//
// Purpose: Syntactic sugar for failures.
...
//
// Purpose: Bridge between go and result types.
...
//
// Purpose: This allows the Result container to be bridged back into standard, idiomatic Go error handling logic (value, err).
// Constraints: None.
// Errors: Returns internal error if any.
...
//
// Purpose: Safely fallback on errors.
// Constraints: If the Result encapsulates an error, it ignores the error and immediately returns the explicitly provided fallback value instead.
// Errors: None.
// Thread-safety: Read-only mapping.
...
//
// Purpose: Lazy evaluation of failure state mapping.
// Constraints: None.
// Errors: None.
...
//
// Purpose: Helper method to retrieve failure context.
// Constraints: None.
// Errors: Returns internal err state.
...
//
// Purpose: Advanced chaining logic.
// Constraints: This enables elegant chaining of multiple operations that might fail. If the original Result is an Err, the error is propagated unchanged.
// Errors: Returns any propagated errors.
// Thread-safety: Generates a new immutable Result. Safe as long as fn is safe.
